...
export default function App() {
  ...
  // --- 초기화 로직 ---
  useEffect(() => {
    // 1. 로컬 백업에서 데이터를 먼저 즉시 불러옴 (로그인 전에도 데이터는 보여야 함)
  ...
  const save = async () => {
    ...
    // [중요] 1. 즉시 로컬 저장 및 화면 업데이트 (서버 응답 기다리지 않음)
    const updatedEntries = editingId 
      ? entries.map(e => e.id === editingId ? payload : e) 
      : [payload, ...entries];
    
    setEntries(updatedEntries);
    localStorage.setItem("archive_full_backup", JSON.stringify(updatedEntries));

    // [중요] 2. 서버 DB 저장 (백그라운드 처리)
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center font-en" style={{ background: activeBg, color: activeText }}>
        <div className="w-64 space-y-6 text-center">
          <h1 className="text-4xl mb-10 font-normal tracking-widest uppercase">Archive</h1>
          ...
          <button onClick={handleLogin} className="w-full mt-4 border border-current rounded-full py-3 text-xs uppercase tracking-widest active:bg-current active:text-white transition-all">Login</button>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {mode === "write" && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end pt-4"><button onClick={save} className="px-12 py-3 border border-current rounded-full text-xs font-en uppercase tracking-widest">Save</button></div>
          </div>
        )}
            ...
            {Object.entries(grouped).map(([title, list]: any) => (
              <section key={title} className="space-y-1">
                ...
                {openWork === title && list.map((e: any) => (
                  <div key={e.id} className="ml-1 pl-4 py-2">
                    <button onClick={() => setOpenEntryId(openEntryId === e.id ? null : e.id)} className="w-full text-left text-[11px] opacity-70">
                      {e.date}{e.time && ` · ${e.time}`}{e.character && ` · ${e.character}`}{e.keywords?.length > 0 && ` · ${e.keywords.join(' ')}`}
                    </button>
                    {openEntryId === e.id && (
                      <div className="pt-3">
                        ...
                        <div className="flex gap-4 mt-4 text-[10px] opacity-40 font-bold font-en uppercase">
                          ...
                          <button onClick={() => {
                            if(confirm("삭제하시겠습니까?")) {
                              ...
                              if (user.id !== 'guest' && e.db_id) supabase.from("entries").delete().eq('id', e.db_id);
                            }
        ...
        {mode === "style" && (
          <div className="space-y-6 max-w-sm">
            ...
            <div className="flex items-center justify-between py-2 border-b border-current/10 font-en">
              ...
              <button onClick={() => {setNight(!night); localStorage.setItem("arch_night", String(!night));}} className={`w-10 h-5 rounded-full relative ${night ? 'bg-blue-500' : 'bg-gray-300'}`}>
        ...
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center p-10" style={{ backgroundColor: activeBg }} onClick={() => setFocusEntry(null)}>
          <div className="max-w-2xl w-full space-y-12 text-center" style={{ color: activeText, fontFamily: koreanFont }}>
            ...
            <div className="text-sm opacity-50 font-en tracking-tight uppercase">
              {focusEntry.work} · {focusEntry.date}{focusEntry.time && ` · ${focusEntry.time}`}{focusEntry.character && ` · ${focusEntry.character}`}{focusEntry.keywords?.length > 0 && ` · ${focusEntry.keywords.join(' ')}`}
            </div>
          </div>
        </div>
      )}
